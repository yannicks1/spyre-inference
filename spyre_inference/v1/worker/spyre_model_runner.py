...
class TorchSpyreModelRunner(GPUModelRunner):
    ...
    def load_model(self, load_dummy_weights: bool = False) -> None:
        ...
        # Move each RoPE module's 2x2 rotation cache onto Spyre once, off the compiled
        # path (modules() dedupes a shared instance by identity). forward_oot then gathers
        # the per-token slice on-device with index_select.
        for rope in self.model.modules():
            if isinstance(rope, _SpyreRotaryMixin):
                rope.prime_device_cache(self._spyre_device)
