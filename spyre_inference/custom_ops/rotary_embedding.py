...
``apply_rope_matmul``). The frequency cache is index_select'd on CPU (Spyre has
no eager index_select): ``_SpyreModelWrapper`` calls ``gather_rotation`` before
the model forward, moves the gathered slice to Spyre, and stashes it in the vLLM
...
When ``head_size // 2`` is not stick-aligned the rotation runs as an
expand -> rotate -> contract matmul: Q/K are padded up to the stick-aligned width,
rotated against an identity-padded cache, then contracted back with a matmul so no
sub-stick strided slice is ever materialized.
...
def _get_expand_contract(
    ...
    ``exp`` is the ``{0, 1}`` matrix [2*inner, 2*padded] that maps each neox half up
    to the stick-aligned ``padded`` on-device via ``x @ exp`` (zero-filling the pad
    lanes); ``con = exp.T`` [2*padded, 2*inner] contracts the rotated result back to
    the original width. Cached per ``(inner, padded, device, dtype)``.
    """
...
def _apply_rope_matmul(
    ...
    """Apply full neox RoPE via per-token 2x2 rotation matrices (ported from
    ...
    the split-half pairing is a pure view; otherwise Q/K are expanded to ``2*padded``
    on-device (``x @ exp``), rotated against an identity-padded cache, then contracted
    back to head_size with ``out @ con`` -- so no sub-stick strided slice is
    materialized. Returns the rotated tensor with ``x``'s shape.
    """
    orig_shape = x.shape
...
class _SpyreRotaryMixin:
    ...
    def _get_rotation_cache(self) -> torch.Tensor:
        ...
        cos_sin_cache ([[cos, -sin], [sin, cos]]), padding the inner dim up to the next
        stick multiple with identity 2x2 blocks so the padded lanes rotate trivially
        (matching hf-adapters; the expand matmul zero-fills those lanes)."""
        if self._rotation_cache is None:
            ...
            if self._padded_inner != inner:
                ident = torch.zeros(
                    cache.shape[0], 2, 2, self._padded_inner - inner, dtype=cache.dtype
                )
                ident[:, 0, 0, :] = 1.0
                ident[:, 1, 1, :] = 1.0
                cache = torch.cat([cache, ident], dim=-1)
